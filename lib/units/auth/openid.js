...
export default (function(options) {
    ...
    // Parse cookies and body BEFORE rate limiting
    ...
    function callbackHandler(req, res) {
        ...
        log.info('Raw cookie header: %s', req.headers.cookie)
        log.info('Parsed req.cookies: %s', JSON.stringify(req.cookies))

        let nonce = req.cookies?.['openid-nonce']

        if (!nonce && req.headers.cookie) {
            const cookies = req.headers.cookie.split(';').reduce((acc, cookie) => {
                const [key, value] = cookie.trim().split('=')
                acc[key] = value
                return acc
            }, {})
            nonce = cookies['openid-nonce']
            log.info('Manually parsed nonce: %s', nonce)
        }

        if (!nonce) {
            log.error('Auth failed! Missed nonce')
            log.error('req.cookies was: %s', JSON.stringify(req.cookies))
            log.error('Cookie header was: %s', req.headers.cookie)
